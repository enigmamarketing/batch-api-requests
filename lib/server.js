    ...
    request(options, function (error, response, body) {
        ...
        if (error || response.statusCode < 200 || response.statusCode > 300) {
            if (self.opts.retries && self.opts.retries < n) {
                ...
                self.logger.error('Boundary:' + self.boundary + ' ID:' + obj.headers['content-id'] + response.statusCode + ' ' + error);
            }
        }
...
Obj.prototype.reply = function (contentId, error, response, body) {
    ...
    message += '\r\n';
    message += '\r\nHTTP/1.1 ' + response.statusCode + ' ' + http.STATUS_CODES[response.statusCode];
